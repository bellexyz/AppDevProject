...
class AuthController extends Controller
{
    ...
    try {
        $validator = Validator::make($request->all(), [
            ...
            'bio_data_path' => $request->input('user_type') === 'artist' ? 'required|file|mimes:pdf,doc,docx|max:4096' : '', // Make bio_data_path optional for listeners
        ]);
    ...
    try {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if ($user->user_type === 'artist') {
                return response()->json(['token' => $token, 'redirect' => '/ArtistProfile'], 200);
